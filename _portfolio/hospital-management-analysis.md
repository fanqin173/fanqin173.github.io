...
plt.show()
```
**图片保存**：`/images/portfolio/hospital-management-analysis/correlation_matrix.png`  
...
### 行动指引
1. **创建文件夹**：在网站项目中创建`images/portfolio/hospital-management-analysis/`目录。
2. **保存图片**：将Notebook中生成的9张可视化图表按上述文件名保存至该目录。
3. **生成MD文件**：创建`_portfolio/hospital-management-analysis.md`文件，粘贴以下完整内容：
...
#### 入院类型与住院时长关系
![入院类型与住院时长关系](/images/portfolio/hospital-management-analysis/admission_stay_relation.png)  
**分析结论**：外伤入院患者住院时长普遍较长，需针对性提升护理效率。

#### 相关性矩阵分析
![相关性矩阵](/images/portfolio/hospital-management-analysis/correlation_matrix.png)  
**分析结论**：床位等级与住院时长呈弱相关，入院押金与患者年龄无显著关联。
...
4. **验证效果**：将文件上传至GitHub Pages，检查页面显示是否正常。
```
